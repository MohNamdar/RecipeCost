...
- سیستم **سختی تهیه از ۱ تا ۵** با هزینه کار ثابت
  ...
  - هزینه کار هر سطح سختی به‌صورت مبلغ ثابت و مستقل قابل تنظیم است.
- **سود اصلی یا سود اختصاصی دسته‌ها** با امکان تنظیم جداگانه برای هر دسته
- **گرد کردن قیمت نهایی** با قابلیت فعال/غیرفعال کردن و تعیین مضرب رند
...
```text
قیمت = (مواد × (۱ + ضایعات) + کار + سربار) × (۱ + سود)
```

سپس اگر قابلیت رند فعال باشد:

```text
قیمت نهایی = رند(قیمت محاسبه‌شده، بر اساس مضرب انتخابی)
```

### هزینه کار / سختی تهیه

سختی دیگر به‌صورت درصد یا ضریب روی کل هزینه اعمال نمی‌شود. برای هر سطح سختی یک مبلغ ثابت قابل تنظیم وجود دارد.

مقادیر پیش‌فرض:

| سختی | هزینه کار (هزار تومان) |
|---:|---:|
| ۱ | ۰ |
| ۲ | ۱۵ |
| ۳ | ۳۰ |
...
یک **سود اصلی** برای کل منو وجود دارد. همچنین می‌توان حالت «سود اختصاصی برای هر دسته» را فعال کرد؛ در این حالت هر دسته درصد سود مستقل دارد و مقدار اولیه هر دسته برابر سود اصلی است.

### گرد کردن

گرد کردن قیمت قابل فعال/غیرفعال کردن است و مضرب آن نیز توسط کاربر تعیین می‌شود؛ مقدار پیش‌فرض مضرب **۵ هزار تومان** است.
